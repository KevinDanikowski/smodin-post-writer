...
import { monthlyDayPositions, dayArray, monthlyDates, hours, minutes } from '../constants'
        ...
        const UserPostTimes = () => {
            ...
            const MonthlyDaySchedules = () => {
                return sortedMonthlyDaySchedules.map((date,index) => {
                    ...
                    const position = monthlyDayPositions.map(dayPosition => {if (dayPosition.number === firstMonthDayNumber) return dayPosition.position})
                    const day = dayArray.map(day => {if (day.number === secondMonthDayNumber) return day.dayShort})
                    return (
